...
// dom references
const selectButton = document.getElementById('start');
const bubbleEl = document.querySelector('.octoo img');
...
function animateBubbles() {
  ...
  // target the <img> inside .octoo and swap its src
  bubbleEl.src = OCTO_FRAMES[currentBubbleFrame];
  currentBubbleFrame = (currentBubbleFrame + 1) % OCTO_FRAMES.length;
...
function startBubbleAnimation() {
  // only start if we have a target element and frames
  if (!bubbleAnimating && bubbleEl && OCTO_FRAMES && OCTO_FRAMES.length) {
...
if (photoboothEl) requestAnimationFrame(animatePhotobooth);
